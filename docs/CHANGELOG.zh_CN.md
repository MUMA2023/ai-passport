...
- 为「迷你高尔夫」加上闲置调光：连续 `GOLF_IDLE_DIM_MS`（60 秒）没有按键就把背光降到 8%，避免这台电池供电的可穿戴设备在没人玩的时候一直亮着屏；下一次按键恢复满亮度。这次唤醒用的按压只负责唤醒、不再转给玩法，否则在时机敏感的力度阶段「按一下让屏幕亮起来」会直接把球打出去。状态翻转逻辑放在无依赖的 `main/golf_idle.{h,c}` 并有主机测试，因为「只在翻转的那一帧上报」两个方向都容易写错 —— 每帧都上报会让背光被反复重设，从不恢复则会让屏幕再也亮不起来。

- 把「迷你高尔夫」固件改成单一玩法启动：去掉 BSP 参考示例的菜单与七个默认演示页（`Display`、`Button`、`Audio`、`Battery`、`Wi-Fi`、`BLE`、`Low Power`）及其 `demo_radio` 辅助模块，`app_main()` 现在初始化板级外设后直接进入球场。`ui_pixel` 主题原样保留 —— 页面仍通过 `ui_pixel_screen_create()` / `ui_pixel_panel_create()` 建屏；`main/demo.h` 保留 `enter`/`exit`/`key` 的页面契约，去掉已无用途的 `demo_entry_t` 注册表。`main` 不再依赖 `bt`、`esp_wifi`、`esp_netif`、`esp_event`、`nvs_flash`。玩法的按键翻译未变；唯一交互变化是长按确定不再有含义，因为已经没有被返回的菜单。

- 新增「迷你高尔夫」玩法：三个布局各异的球洞（2 杆洞「Side Step」用左上角沙坑惩罚瞄太高、右下角立柱惩罚拉平；3 杆洞「Dogleg」用上沿矮墙与拐角水塘封住抄近路；3 杆洞「Pond Side」用右侧大片水塘守住果岭下方），只用三个实体按键的两段式「瞄准—挥杆」操作，以及按地形区分的滚动摩擦。球场模型、墙体与边界反弹、带溜边阈值的进洞判定、落水罚杆、计分，以及逐像素的场景/叠加渲染全部为整数定点实现并有主机测试覆盖；另有一条主机回归测试断言每洞默认的开球瞄准线上既没有水也没有墙，避免日后改球场布局时把第一杆悄悄变成设计陷阱。音效复用俄罗斯方块示例里的异步 RTTTL 播放器。按键翻译单独抽成有主机测试的一层（`golf_input`）：button 组件在按下瞬间先报 `PRESS_DOWN`，长按阈值到了才报 `LONG_PRESS_START`，抬起后还可能补一个 `SINGLE_CLICK`，所以原先一次长按会同时施加微调与粗调（36 度而不是 32 度），击球后补的那个 CLICK 还会自己把球推进下一洞。
